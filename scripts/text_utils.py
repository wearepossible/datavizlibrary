...
Used by:
  - scripts/extract_text.py (batch processing)
  - admin/admin.py (future, for new uploads)
"""
...
# Point pytesseract to Homebrew's Tesseract if not on PATH
...
def ocr_png(png_path):
    """Run Tesseract OCR on a PNG file."""
    try:
...
def extract_text_for_record(record, image_dir):
    ...
    Strategy: prefer SVG XML text (fast, accurate).
    Fall back to PNG OCR if SVG has no text elements.
    """
    image_dir = Path(image_dir)

    # Try SVG XML extraction first
    ...
    # Last resort: rasterize SVG and OCR
    for svg_file in record.get("svg_files", []):
        svg_path = image_dir / svg_file
        if svg_path.exists():
